import 'server-only';
import type { Promotion, PromotionType, SaleChannel } from '@prisma/client';
import { db } from '../db';
import { round, roundAmount, roundPrice, taxFromGross, toNumber } from '../money';
...
interface LoadedPromotion extends Promotion {
  targets: { variantId: string | null; categoryId: string | null; brandId: string | null; isExclusion: boolean; isRewardSide: boolean }[];
}
...
function matchesTimeWindow(promotion: Promotion, at: Date): boolean {
...
function lineMatches(promotion: LoadedPromotion, line: BasketLine, rewardSide = false): boolean {
